...
CheckList::CheckList(int height, int width, vector<string> strings) : Panel(height, width)
{
	...
	lastChanged = (Panel*)getControllers()[0];
...
Panel * CheckList::getLastChanged()
{
	return lastChanged;
}

void CheckList::setLastChanged(Panel * lc)
{
	lastChanged = lc;
}
...
{
	lastChanged->SetBackground(BackgroundColor::White);
	lastChanged->SetForeground(ForegroundColor::Black);
	lastChanged->draw();
	SetConsoleCursorPosition(cursorHandler, { lastChanged->getLocationX() + 1, lastChanged->getLocationY() + 1 });
}

void CheckList::focusOut()
{
	lastChanged->SetBackground(backgroundColor);
	lastChanged->SetForeground(foregroundColor);
	lastChanged->draw();
}
